...
export async function POST(request: Request) {
  ...
  if (!session || !hasPermission(session, "VENDOR_CREATE")) return NextResponse.json({ ok: false, message: "Forbidden" }, { status: 403 });
  try {
    ...
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Unable to create vendor." }, { status: 400 });
  }
}
