...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  if (!session || !hasPermission(session, "VENDOR_EDIT"))
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "Unable to update vendor." },
      { status: 400 }
    );
  }
}
...
export async function DELETE(_: Request, { params }: RouteParams) {
  const session = await getSession();
  if (!session || !hasPermission(session, "VENDOR_DELETE"))
    return NextResponse.json({ ok: false, message: "Forbidden" }, { status: 403 });
  try {
    const { id } = await params;
    ...
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "Unable to delete vendor." },
      { status: 400 }
    );
  }
}
