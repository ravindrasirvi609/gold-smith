...
export default async function EditIssuePage({ params }: PageProps) {
  ...
  if (!session || !hasPermission(session, "ISSUE_EDIT"))
    redirect("/dashboard/manufacturing/issues");
  const { id } = await params;
  const [issue, karigars] = await Promise.all([
    getIssueById(id),
    getKarigarOptions(),
  ]);
  if (!issue) notFound();
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-6 py-10">
        <div className="w-full">
          <KarigarIssueForm
            mode="edit"
            actionUrl={`/api/karigar-issues/${id}`}
            karigars={karigars.map((k) => ({ id: k.id, name: k.name }))}
            initialValues={issue}
            canDelete={hasPermission(session, "ISSUE_DELETE")}
          />
        </div>
      </div>
    </main>
