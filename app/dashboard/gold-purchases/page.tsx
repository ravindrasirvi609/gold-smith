...
export default async function GoldPurchasesPage({ searchParams }: PageProps) {
  const session = await getSession();
  if (!session || !hasPermission(session, "PURCHASE_VIEW")) redirect("/dashboard");
  const query = parseListQuery(await searchParams);
  const [result, summary] = await Promise.all([
    getGoldPurchases(query),
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight">Gold purchases</h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Ledger-backed gold procurement.
            </p>
          </div>
          {hasPermission(session, "PURCHASE_CREATE") ? (
            <Link href="/dashboard/gold-purchases/new">
              <Button>Create gold purchase</Button>
            </Link>
          ) : null}
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border bg-card p-4">
            ...
            <p className="mt-2 text-2xl font-semibold">
              {Number(summary.currentStock ?? 0).toFixed(2)}
            </p>
          </div>
          <div className="rounded-2xl border bg-card p-4">
            <p className="text-sm text-muted-foreground">Total in</p>
            <p className="mt-2 text-2xl font-semibold">
              {Number(summary.totalIn ?? 0).toFixed(2)}
            </p>
          </div>
          <div className="rounded-2xl border bg-card p-4">
            <p className="text-sm text-muted-foreground">Total out</p>
            <p className="mt-2 text-2xl font-semibold">
              {Number(summary.totalOut ?? 0).toFixed(2)}
            </p>
          </div>
        </div>
                  ...
                  <tr key={purchase.id} className="border-b last:border-b-0">
                    ...
                    <td className="px-4 py-4">
                      {hasPermission(session, "PURCHASE_EDIT") ? (
                        <Link
                          href={`/dashboard/gold-purchases/${purchase.id}/edit`}
                          className="text-sm underline underline-offset-4"
                        >
                          Edit
                        </Link>
